...
public sealed class MediatRTests
{
    ...
    {
        //GIVEN
        var types = Assembly.Load(new AssemblyName("SilentMike.DietMenu.Mailing.Application")).GetTypes().ToList();
        types.AddRange(Assembly.Load(new AssemblyName("SilentMike.DietMenu.Mailing.Infrastructure")).GetTypes());
