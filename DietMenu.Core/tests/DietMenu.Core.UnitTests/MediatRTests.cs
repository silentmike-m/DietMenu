namespace DietMenu.Core.UnitTests;
...
public sealed class MediatRTests
{
    ...
    {
        //GIVEN
        var types = Assembly.Load(new AssemblyName("DietMenu.Core.Application")).GetTypes().ToList();
        types.AddRange(Assembly.Load(new AssemblyName("DietMenu.Core.Infrastructure")).GetTypes());
