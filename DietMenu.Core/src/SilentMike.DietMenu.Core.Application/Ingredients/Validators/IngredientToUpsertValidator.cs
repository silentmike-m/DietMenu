...
internal sealed class IngredientToUpsertValidator : AbstractValidator<IngredientToUpsert>
{
    public IngredientToUpsertValidator()
    {
        RuleFor(pt => pt.Exchanger)
            ...
            .WithErrorCode(ValidationErrorCodes.UPSERT_INGREDIENTS_INVALID_EXCHANGER)
            .WithMessage(ValidationErrorCodes.UPSERT_INGREDIENTS_INVALID_EXCHANGER_MESSAGE)
            ;

        RuleFor(pt => pt)
            .Must(pt => pt.Name is null || !string.IsNullOrWhiteSpace(pt.Name))
            .WithErrorCode(ValidationErrorCodes.UPSERT_INGREDIENTS_EMPTY_NAME)
            .WithMessage(ValidationErrorCodes.UPSERT_INGREDIENTS_EMPTY_NAME_MESSAGE)
            ;

        RuleFor(pt => pt)
            .Must(pt => pt.UnitSymbol is null || !string.IsNullOrWhiteSpace(pt.UnitSymbol))
            .WithErrorCode(ValidationErrorCodes.UPSERT_INGREDIENTS_EMPTY_UNIT)
            .WithMessage(ValidationErrorCodes.UPSERT_INGREDIENTS_EMPTY_UNIT_MESSAGE)
            ;
    }
}
