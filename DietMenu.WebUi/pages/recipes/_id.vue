...
export default Vue.extend({
  ...
  setup() {
    ...
    const { addIngredients, getNewRecipe, getRecipe, saveRecipe } = useRecipe();
    ...
    onMounted(() => {
      ...
      if (id === "new") {
        ...
        setBreadcrumbs(BreadcrumbType.RecipeDetails, "Nowy");
      } else {
        getRecipe(id)
          .then((result) => {
            recipe.value = result;
            setBreadcrumbs(BreadcrumbType.RecipeDetails, recipe.value.name);
          })
    ...
    const deleteRow = (row: RecipeIngredient): void => {
    ...
    const save = () => {
      saveRecipe(recipe.value).then((response) => {
        if (response) close();
      });
    };

    const showReplacements = (row: RecipeIngredient) => {
      showGridDialog(
        new GridDialogOptions(
          "Zamienniki",
          false,
